...
export const createOrder = TryCatch(async(req:AuthenticatedRequest,res)=>{
    ...
    const a = Math.sin(dLat/2)*Math.sin(dLat/2)*Math.cos((lat2*Math.PI)/180)*Math.sin(dLon/2)*Math.sin(dLon/2);
...
{
    console.log("Publishing order as ready for the rider" , order._id);
...
export const updateOrderStatusForRider = TryCatch(async(req,res)=>{
    if(req.headers["x-internal-key"] !== process.env.INTERNAL_SERVICE_KEY)
    {
        return res.status(403).json({
            message : "Forbidden",
