      ...
      <div className="space-y-4">
        {
          cart.map((cartItem : ICart)=>{
              ...
              <div className="flex items-center gap-3">
                <button className="rounded-full border p-2 hover:bg-gray-100 disabled:opacity-50" disabled={isLoading}
          ...
          <div className="rounded-xl bg-white p-4 shadow-sm space-y-3">
                <div className="flex justify-between text-sm">
                    <span>Total Items</span> <span>{quantity}</span>
                </div>
        ...
        <button onClick={checkout} className={`mt-3 w-full rounded-lg bg-[#E23774] py-3 text-sm font-semibold text-white hover:bg-red-800 ${!restaurant.isOpen ? "opacity-50" : ""}`}>
          {!restaurant.isOpen ? "Restaurant is Closed" : "Proceed to checkout"}
        </button>
         <button onClick={clearCart} disabled={clearingCart} className="mt-3 w-full rounded-lg bg-[#7b7979] py-3 text-sm font-semibold text-white hover:bg-gray-900 flex justify-center items-center gap-3">
