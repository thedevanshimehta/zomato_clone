...
onItemDeleted();
  ...
  console.log(error);
  toast.error("Uanble to delete item");
}

}

const handleAvailability = async (itemId : string) => {
...
onItemDeleted();
  ...
  console.log(error);
  toast.error("Uanble to change item status");
}
          ...
          return (
            <div className={`relative flex gap-4 rounded-lg bg-white p-4 shadow-sm transition ${
              ...
              <div className="relative shrink-0">
                ...
                {
                  !item.isAvailable && (
                    <span className="absolute inset-0 flex items-center justify-center rounded bg-black/60 text-xs font-semibold text-white">Not Available</span>
                  )
                }
              </div>
                 ...
                 ₹ {item.price}
...

                  {
                    isSeller && (
                      <div className="flex gap-2">
                        <button onClick={()=>handleAvailability(item._id)} className="rounded-lg p-2 text-gray-600 hover : bg-gray-100">
                          {item.isAvailable ? (
                            <BsEye size={18}/>
                          ): (
                            <FiEyeOff size={18}/>
                  ...
                  }
                  {
                    !isSeller && (
                      ...
                      onClick={()=>addToCart(item.restaurantId , item._id)} className={`flex items-center justify-center rounded-lg p-2 ${!item.isAvailable || isLoading ? "cursor-not-allowed text-gray-400 " : "text-red-500 hover : bg-red-50"}`}>
                    {
